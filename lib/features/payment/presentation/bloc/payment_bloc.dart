...
import '../../data/models/payment_with_zero_amount.dart';
...
class PaymentBloc extends Bloc<PaymentIntentEvent, PaymentIntentState> {
  ...
  PaymentBloc() : super(const PaymentIntentState()) {
    ...
    on<PaymentInitiatedZeroAmount>((event, emit) async {
      emit(state.copyWith(theState: TheStates.loading));
      try {
        await repositories.postPaymentZeroAmount(event.uuid).then(
              (value) => emit(
                state.copyWith(
                  theState: TheStates.success,
                  paymentZero: PaymentWithZeroAmount.fromJson(value),
                ),
              ),
            );
      } catch (e) {
